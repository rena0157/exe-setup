...
- **CLI toolchain**: `ripgrep`, `fd`, `eza`, `ast-grep`, `bat`, `fzf`, `zoxide`, `delta`, `jq`, `yq`, `gh`, `btop`
...
- Debian/Ubuntu (uses `apt-get` for Homebrew prerequisites), or a system with Homebrew already on `PATH`
